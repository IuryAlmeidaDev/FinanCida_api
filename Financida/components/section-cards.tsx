...
export function SectionCards({
  ...
  const monthLabel = new Date(range.year, range.month - 1, 1).toLocaleDateString(
    "pt-BR",
    {
      month: "long",
      year: "numeric",
    ...
    <div className="grid grid-cols-1 gap-3 px-4 lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      <Card className="@container/card border-emerald-100 bg-gradient-to-br from-white to-emerald-100/80 transition-colors hover:to-emerald-200/70 dark:border-emerald-900/60 dark:from-card dark:to-emerald-950/40 dark:hover:to-emerald-900/50">
        <CardHeader className="gap-2 p-4 pb-2">
          ...
          <CardTitle className="text-xl font-semibold tabular-nums text-foreground/85 @[250px]/card:text-2xl">
            {moneyFormatter.format(summary.totalRevenue)}
          </CardTitle>
          <CardAction>
            <Badge
              variant="outline"
              className="border-emerald-200 bg-white text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-200"
            >
              <TrendingUpIcon />
              Receitas
            </Badge>
        ...
        <CardFooter className="flex-col items-start gap-1 text-xs text-muted-foreground">
          ...
          <div className="text-muted-foreground">Referente a {monthLabel}</div>
        </CardFooter>
      </Card>
      <Card className="@container/card border-sky-100 bg-gradient-to-br from-white to-sky-100/80 transition-colors hover:to-sky-200/70 dark:border-sky-900/60 dark:from-card dark:to-sky-950/40 dark:hover:to-sky-900/50">
        <CardHeader className="gap-2 p-4 pb-2">
          ...
          <CardAction>
            <Badge
              variant="outline"
              className="border-sky-200 bg-white text-sky-700 dark:border-sky-800 dark:bg-sky-950/30 dark:text-sky-200"
            >
              <ExpenseTrendIcon />
        ...
        <CardFooter className="flex-col items-start gap-1 text-xs text-muted-foreground">
          ...
          <div className="text-muted-foreground">
            Despesas fixas e variáveis no período
          </div>
        </CardFooter>
      </Card>
      <Card className="@container/card border-emerald-100 bg-gradient-to-br from-white to-emerald-100/80 transition-colors hover:to-emerald-200/70 dark:border-emerald-900/60 dark:from-card dark:to-emerald-950/40 dark:hover:to-emerald-900/50">
        ...
        <CardFooter className="flex-col items-start gap-1 text-xs text-muted-foreground">
          ...
          <div className="text-muted-foreground">Indicador de saúde financeira</div>
        </CardFooter>
      </Card>
      <Card className="@container/card border-amber-100 bg-gradient-to-br from-white to-amber-100/80 transition-colors hover:to-amber-200/70 dark:border-amber-900/60 dark:from-card dark:to-amber-950/40 dark:hover:to-amber-900/50">
        <CardHeader className="gap-2 p-4 pb-2">
          ...
          <CardAction>
            <Badge
              variant="outline"
              className="border-red-200 bg-white text-red-700 dark:border-red-900/70 dark:bg-red-950/30 dark:text-red-200"
            >
              <TrendingDownIcon />
        ...
        <CardFooter className="flex-col items-start gap-1 text-xs text-muted-foreground">
          <div className="line-clamp-1 flex gap-2 font-medium">
            Despesas variáveis, em aberto e atrasadas{" "}
